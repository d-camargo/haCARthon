...
| Canal de entrega | **Híbrido: plugin QGIS (analista) + painel web (gestão)**; plugin novo e independente (escopo próprio: análise); **native-first** (Processing + UX nativa do QGIS; única GUI custom = painel de KPIs); QGIS puro com PostGIS opcional; MVP = detecção + fila + parecer/RAT | Decidido 2026-06-25 — arquitetura em `docs/arquitetura/plugin-qgis.md`; análise em `docs/definir/opcao-entrega-qgis-vs-web.md` |
