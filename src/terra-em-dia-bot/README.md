...
- **Mapas com imagem de satélite:** Os mapas gerados para o produtor rural (`mapa.py`) utilizam a imagem de satélite **Esri World Imagery** como fundo, com alinhamento universal de vetores em projeção métrica. O motor de mapas foi migrado 100% para o sistema de coordenadas UTM (SIRGAS 2000 no fuso correspondente à longitude do imóvel). A requisição de exportação de satélite é feita com parâmetros `bboxSR` e `imageSR` em UTM e formato `f=json` ao Esri para ler o `extent` geográfico real de posicionamento retornado, aplicando-o no `imshow` do Matplotlib (com `ax.set_aspect("equal")`). O mapa comparativo inclui uma cota e seta indicativas de "Faixa legal: 30m da margem".
- **Camada Visual de Hidrografia:** O mapa realiza uma consulta em tempo real à API REST do GeoPR (IAT) no serviço de cursos d'água ZEE-PR (`zee_rios`) usando o bbox em UTM da propriedade. Se disponível, desenha o traçado das linhas do rio (em azul claro) como camada de referência visual, auxiliando a visualização espacial da mata ciliar em relação ao leito regular.
