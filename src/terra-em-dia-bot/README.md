...
- **Mapas com imagem de satélite:** Os mapas gerados para o produtor rural (`mapa.py`) utilizam a imagem de satélite **Esri World Imagery** como fundo, com alinhamento universal de vetores em projeção métrica. O motor de mapas opera no sistema de coordenadas UTM (SIRGAS 2000 no fuso correspondente à longitude do imóvel). A requisição de exportação de satélite é feita com parâmetros `bboxSR` e `imageSR` em UTM e formato `f=json` ao Esri para ler o `extent` geográfico real de posicionamento retornado, aplicando-o no `imshow` do Matplotlib (com `ax.set_aspect("equal")`). O deslocamento (offset) residual eventual provém do erro intrínseco de ortorretificação do satélite base em áreas rurais ou de precisão do GPS na declaração original do CAR, não constituindo falha no código do bot.
- **Polígono Dinâmico da APP Ideal:** O mapa comparativo gera e desenha no painel de solução ("Em dia") uma geometria de APP expandida por um buffer espacial de 13 metros gerado dinamicamente via OGR sobre a própria APP declarada do produtor, intersectando-o com o perímetro do imóvel. Isso permite demonstrar visualmente e de forma didática a recomposição da faixa ideal de 30 metros de mata ciliar mantendo-se estritamente dentro dos limites da propriedade, sem riscos de deslocamento causados por bases externas de hidrografia.
